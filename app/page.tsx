...
function getWeekStats(sessions: Session[], now = new Date()) {
  ...
  for (const session of inWeek) {
    const hasSeries = session.exercises.some((exercise) => exercise.sets.length > 0);
    if (hasSeries) trainedDates.add(localDateKey(session.startedAt));
...
export default function Home() {
  ...
  function finishSession() {
    ...
    setActiveExerciseId(null);
    setTab('today');
    notify('Sessão finalizada e salva.');
  }
  ...
  function handleCatalogPick(exercise: CatalogExercise) {
    ...
    if (pickerMode === 'swap' && pickerSessionExerciseId) {
      ...
      setComposerReps('10');
      setModal(null);
      notify('Exercício trocado. Registre a primeira série.');
      return;
    }
    ...
    setComposerReps('10');
    setModal(null);
    notify('Exercício adicionado. Registre a primeira série.');
  }
      ...
      <main className="app-main">
        ...
        {pinnedPlan && (
          <div className="surface pin-card">
            ...
            <h2 className="pin-name">{pinnedPlan.name}</h2>
        ...
        {!state.todayPin && state.plans.length > 0 && (
          <section>
            ...
            <div className="list">{state.plans.slice(0, 3).map((plan) => <div className="list-card" key={plan.id}><div className="list-card-main"><h3>{plan.name}</h3><p>{plan.exercises.length} exercícios · atualizado {formatDate(plan.updatedAt)}</p></div><button className="btn btn-secondary btn-small" type="button" onClick={() => pinPlan(plan.id)}><Pin size={14} /> Fixar</button></div>)}</div>
          </section>
        )}
    ...
    return (
      <div className="session-shell">
        <main className="session-main">
          <div className="session-header"><div><button className="btn btn-quiet btn-small" type="button" onClick={() => { setSessionViewId(null); setActiveExerciseId(null); }}><ArrowLeft size={16} /> Voltar</button><p className="eyebrow" style={{ marginTop: 17 }}>Sessão em andamento</p><h1>{activeSession.sourcePlanName ?? 'Sessão vazia'}</h1><p>Começou {formatDateTime(activeSession.startedAt)}</p></div><button className="btn btn-danger btn-small" type="button" onClick={() => discardSession()}><Trash2 size={14} /> Descartar</button></div>
