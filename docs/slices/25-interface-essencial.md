...
- Issue: pendente — vincular uma issue de intenção antes de promover a `ready`; não é a #1, #6, #7 nem #8.
- Status: draft
- Cabe no próximo treino? sim — a primeira entrega simplifica a entrada e recolhe a navegação secundária.
- Direção visual aprovada pelo dono em 06/09/2026. A aprovação do visual não promove este plano a `ready`.
...
A referência principal é o mock final desta conversa: fundo escuro uniforme, texto marfim, título serifado, botão claro “Começar” e nenhuma barra inferior. O menu lateral aprovado anteriormente permanece como padrão de navegação, adaptado à nova paleta sóbria.
